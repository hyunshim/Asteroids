...
class Observable<Input> {
   ...
   */
  subscribe(next: (_: Input) => void, complete?: () => void, error?: () => void): () => void {
  ...
  /**
   * creates a child Observable that is detached when the given condition is true
   * @param condition filter predicate
   * @return child Observable of notifications while condition is true
   */
  takeWhile(condition: (_: Input) => boolean): Observable<Input> {
    return new Observable<Input>(observer => {
