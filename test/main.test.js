...
// Extra tests added by: Hyun Shim (lines 310 - 358)
...
describe("Extra tests added by Hyun Shim", function () {
  it("takeWhile - test basic functionality", function (done) {
    const testTime = 100
    let testBoolean = true
    let i = 0
    Observable
      .interval(10)
      .takeWhile(() => testBoolean) // Call next if testBoolean is true; otherwise complete
      .subscribe((interval) => i = interval) // Set i to interval

    setTimeout(() => {
      testBoolean = false // testBoolean will be set to false after the specified testTime; this will complete the Observable

      try {
        expect(i).to.equal(testTime) // If takeWhile has worked properly, the Observable should have run for the specified testTime
      } catch (error) {
        done(error);
        return;
      }
      done();
    }, testTime+5) // Added 5ms as sometimes it would check before the i has been set to the new interval

  })

  it("takeWhile - test whether (x,y) coordinate is on screen", function (done) {
    const 
      bounds = { width: 10, height: 10 },
      xy = { x: 0, y: 0 },
      expected = {x: 10, y: 10},
      inScreen = (x, y) => !(x >= bounds.width || y >= bounds.height); // Copied over from asteroids as importing asteroids.js caused errors due to no svg for canvas
    
    Observable
      .interval(10)
      .takeWhile(() => inScreen(xy.x, xy.y))
      .subscribe(() => {
        xy.x++; xy.y++
      })
    ...
    setTimeout(() => {
      try {
        expect(xy).to.deep.equal(expected)
      } catch (error) {
        done(error);
        return;
      }
      done();
    }, 200) // Since this is running for 200ms, if takeWhile is not working properly; xy should be (20, 20)
  })
})